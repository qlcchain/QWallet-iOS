...
#ifdef PrintLog
        self.logTimer = [NSTimer scheduledTimerWithTimeInterval:5.f target:self selector:@selector(printLog) userInfo:nil repeats:YES];
#endif
...
- (void)printLog {
    dispatch_async(dispatch_get_main_queue(), ^{
        NSLog(@"backgroundTimeRemaining~~~~~~~~~~~~~~~~~~%.2f", [UIApplication sharedApplication].backgroundTimeRemaining);
    });
}
...
- (void)setUpAudioSession {
    ...
    NSLog(@"audioSession.isOtherAudioPlaying = %d", audioSession.isOtherAudioPlaying);
