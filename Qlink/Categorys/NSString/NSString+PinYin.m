...
- (NSArray<AreaCodeModel *> *)areaCodeModelArrayWithPinYinFirstLetterFormat {
    ...
    for (AreaCodeModel *model in self) {
        NSString *firstLetter = [model.en getFirstLetter];
        NSMutableArray *array = dict[firstLetter];
    ...
    for (int i = 'A'; i <= 'Z'; i++) {
        ...
        if ([array count]) {
            [array sortUsingComparator:^NSComparisonResult(id obj1, id obj2) {
                AreaCodeModel *model1 = obj1;
                AreaCodeModel *model2 = obj2;
                return [model1.en localizedCompare:model2.en];
            }];
            NSDictionary *resultDict = @{@"firstLetter": firstLetter,
                                         @"content": array};
            [resultArray addObject:resultDict];
