...
@interface InviteFriendOutbreakViewController ()
...
@implementation InviteFriendOutbreakViewController
...
//    _qrImgV.image = [UIImage imageNamed:@"share_download.jpg"];
    ...
    if ([language isEqualToString:LanguageCode[0]]) { // 英文
        _inviteBackImg.image = [UIImage imageNamed:@"me_Invitation_en"];
        shareUrl = @"https://qwallet.network/en";
    } else if ([language isEqualToString:LanguageCode[1]]) { // 中文
        _inviteBackImg.image = [UIImage imageNamed:@"me_Invitation_ch"];
