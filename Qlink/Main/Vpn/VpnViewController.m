...
//    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(checkProcessSuccessOfVPNSeize:) name:CHECK_PROCESS_SUCCESS_VPN_SEIZE object:nil];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(getProfileTimeout:) name:Get_Profile_Timeout_Noti object:nil];
    // 修改本地资产成功通知
...
- (void)handleSourceWithConnectVpn {
    ...
    if (connectInfo && (connectInfo.isMainNet != [ConfigUtil isMainNetOfServerNetwork])) {
        return;
    }
    ...
    [self.sourceArr enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        VPNInfo *vpnInfo = obj;
        if ([vpnInfo.vpnName isEqualToString:[NeoTransferUtil currentVPNName]]) {
            connectIsExist = YES;
        }
    }];
...
//        vpnInfo.connectStatus = VpnConnectStatusNone;
        if (_isConnectVPN) {
            NSString *currentConnnectVPN = [NeoTransferUtil currentVPNName];
...
- (void)clickConversation:(VPNInfo *)vpnInfo {
    if ([self vpnIsMine:vpnInfo.p2pId]) { // 自己的vpn资产
        ...
        _joinGroupFlag = NO;
        [kAppD.window makeToastInView:self.view];
...
- (void)getChatGroupToJump:(VPNInfo *)vpnInfo {
    ...
    if (chatM) {
        ...
        [kAppD.window makeToastInView:self.view text:NSStringLocalizable(@"group_invalid") userInteractionEnabled:YES hideTime:2];
...
- (void)joinGroupTimeout {
    if (!_joinGroupFlag) {
        ...
        [kAppD.window makeToastDisappearWithText:NSStringLocalizable(@"timeout")];
    }
}

- (void)showConnectJudge:(VPNInfo *)vpnInfo {
    WalletCommonModel *currentWalletM = [WalletCommonModel getCurrentSelectWallet];
    if ([vpnInfo.address isEqualToString:currentWalletM.address?:@""]) { // 连接自己钱包的VPN
...
//                [NEOWalletUtil checkWalletPassAndPrivateKey:self TransitionFrom:CheckProcess_VPN_CONNECT];
//                [self performSelector:@selector(checkVPNConnect) withObject:self afterDelay:0.6];
                [self checkVPNConnect];
            }];
        } else {
//            [NEOWalletUtil checkWalletPassAndPrivateKey:self TransitionFrom:CheckProcess_VPN_CONNECT];
//            [self performSelector:@selector(checkVPNConnect) withObject:self afterDelay:0.6];
...
- (void)vpnStatusChange:(NSNotification *)noti {
    ...
    switch (status) {
        ...
        case NEVPNStatusConnected:
        {
            if (![[WalletCommonModel getCurrentSelectWallet].address isEqualToString:_selectVPNInfo.address]) {
                _selectVPNInfo.isMainNet = [ConfigUtil isMainNetOfServerNetwork];
                [NeoTransferUtil udpateTransferModel:_selectVPNInfo];
            }
...
//                [self addNewGuideVpnListConnect];
            ...
            if (![self selectVpnIsMine]) {
                // vpn连接成功进行转账
                // [NeoTransferUtil sendFundsRequestWithType:3 withVPNInfo:_selectVPNInfo];
            }
        }
            break;
...
- (void)getProfileTimeout:(NSNotification *)noti {
    _selectVPNInfo.connectStatus = VpnConnectStatusNone;
    [self refreshTable];
}
...
//    CGRect hollowOutFrame = [_myAssetsBtn.superview convertRect:_myAssetsBtn.frame toView:[UIApplication sharedApplication].keyWindow];
//    kWeakSelf(self);
...
//    CGFloat y = IS_iPhone_X?96 + 24:96;
...
//    CGFloat y = IS_iPhone_X?139 + 24:139;
...
//        _myAssetsBtn.hidden = YES;
//        _myAssetsBtn.enabled = NO;
//    } else {
//        _myAssetsBtn.hidden = NO;
//        _myAssetsBtn.enabled = YES;
//    }
}
    ...
    [cell setConnectClickB:^{
        ...
        if (vpnInfo.connectStatus == VpnConnectStatusNone) { // 选中cell未连接
            if (vpnInfo.online <= 0) { // offline 不能点击
                [kAppD.window makeToastDisappearWithText:NSStringLocalizable(@"friend_unline_p2p")];
                return;
            }
            ...
            if (_isConnectVPN) { // 当前app已连接vpn
                ...
                [weakself showConnectJudge:vpnInfo];
            }
...
//    VPNInfo *vpnInfo = _sourceArr[indexPath.row];
//    if (vpnInfo.online <= 0) { // offline 不能点击
//        [kAppD.window showHint:NSStringLocalizable(@"friend_unline_p2p")];
//        return;
//    }
...
#pragma mark - Action
- (IBAction)myAssetsAction:(id)sender {
    if (kAppD.allowPresentLogin) {
        kWeakSelf(self)
        [kAppD presentLogin:^{
            [weakself myAssetsAction:nil];
        }];
        return;
    }
...
- (ChooseCountryView *)countryView
{
    if (!_countryView) {
        ...
        _countryView.bgContraintTop.constant = 44;
